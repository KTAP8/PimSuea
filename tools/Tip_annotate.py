...
# ─── CONFIG ──────────────────────────────────────────────────────────────────
INPUT_PATH = "/Volumes/My Passport/Personal_Project/PimSuea/tools/real_data/warm_shirt_black.png"   # ← set this
OUTPUT_PATH = None                        # None = auto: <input>_annotated.png
PHYSICAL_W_IN = 12.0                        # inches
PHYSICAL_H_IN = 16.0                        # inches

# pixels with alpha <= this are treated as empty (higher = ignores antialiased edges)
ALPHA_THRESHOLD = 200
...
MOCKUP_IMAGE_PATH = "/Volumes/My Passport/Personal_Project/PimSuea/tools/templates/front_white_mock_template.png"
...
# ─── BATCH CONFIG ────────────────────────────────────────────────────────────
BATCH_SIDES = [
    {"input": "/Volumes/My Passport/Personal_Project/PimSuea/tools/real_data/a76ed163-1947-42a3-9bab-a8200cdb5421/front.png",
        "mockup": "/Volumes/My Passport/Personal_Project/PimSuea/tools/templates/merch_white_front.JPG", "side": "front", "show_tag": False, "text_color": "red"},
...
def get_print_tier(w_in: float, h_in: float) -> str:
    ...
    if short <= 3.0 and long_ <= 4.0:
        return "3x4in"
    if short <= 6.0 and long_ <= 8.0:
        return "A5"
    if short <= 8.0 and long_ <= 12.0:
        return "A4"
    return "A3"
...
def find_content_bbox(img: Image.Image):
    ...
    if not rows.any():
        raise ValueError(
            "No content found in the print file — image appears empty.")
...
def draw_label(draw, text, cx, cy, font, fill=(0, 0, 0, 255)):
    """Draw text centered at (cx, cy), no background."""
    bbox = draw.textbbox((0, 0), text, font=font)
    tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
    draw.text((cx - tw // 2, cy - th // 2), text, fill=fill, font=font)


def draw_dimension_bracket(draw, x1, y1, x2, y2, label, font, orientation,
                           color=ORANGE, label_fill=(0, 0, 0, 255)):
    ...
    if orientation == 'horizontal':
        ...
        draw_label(draw, label, (x1 + x2) // 2, y1 + PAD +
                   FONT_SIZE // 2, font, fill=label_fill)
    else:  # vertical
        draw.line([(x1 - TICK, y1), (x1 + TICK, y1)], fill=color, width=LINE_W)
        draw.line([(x2 - TICK, y2), (x2 + TICK, y2)], fill=color, width=LINE_W)
        draw_label(draw, label, x1 + PAD + FONT_SIZE // 2,
                   (y1 + y2) // 2, font, fill=label_fill)
...
def composite_on_mockup(
    ...
    label_fill = _TEXT_COLORS.get(
        text_color or TEXT_COLOR, _TEXT_COLORS["black"])
    mp = Path(mockup_path)
    if not mp.exists():
    ...
    draw.line([(collar_x, collar_y), (collar_x, elem_top_y)],
              fill=ORANGE, width=LINE_W)
    draw.line([(collar_x - TICK, collar_y),    (collar_x +
              TICK, collar_y)],    fill=ORANGE, width=LINE_W)
    draw.line([(collar_x - TICK, elem_top_y),  (collar_x +
              TICK, elem_top_y)],  fill=ORANGE, width=LINE_W)
    y_total_label = format_krabiad(measurements['collar_to_elem_in'])
    draw_label(draw, y_total_label, collar_x - TICK - 30,
               collar_y + FONT_SIZE // 2 + 4, font, fill=label_fill)

    # ── X measurement: shirt center → nearest edge of design (front and back) ──
    x_dir = measurements["x_dir"]
    if x_dir != "centered":
        draw_dashed_line(draw, collar_x, elem_top_y,
                         nearest_x_px, elem_top_y, color=ORANGE)
        draw.line([(collar_x,     elem_top_y - TICK), (collar_x,
                  elem_top_y + TICK)], fill=ORANGE, width=LINE_W)
        draw.line([(nearest_x_px, elem_top_y - TICK), (nearest_x_px,
                  elem_top_y + TICK)], fill=ORANGE, width=LINE_W)
        x_label_text = f"{format_krabiad(measurements['x_offset_in'])} {x_dir}"
        mid_h = (collar_x + nearest_x_px) // 2
        draw_label(draw, x_label_text, mid_h, elem_top_y -
                   FONT_SIZE // 2 - 4, font, fill=label_fill)
    else:
        draw_label(draw, "centered", collar_x - TICK - 30,
                   elem_top_y - FONT_SIZE // 2 - 4, font, fill=label_fill)
    ...
    if _tag_info is not None:
        ...
        draw.line([(tag_line_x, TAG_COLLAR_Y_FRONT),
                  (tag_line_x, ty)], fill=ORANGE, width=LINE_W)
        draw.line([(tag_line_x - TICK, TAG_COLLAR_Y_FRONT), (tag_line_x +
                  TICK, TAG_COLLAR_Y_FRONT)], fill=ORANGE, width=LINE_W)
        draw.line([(tag_line_x - TICK, ty),                 (tag_line_x +
                  TICK, ty)],                 fill=ORANGE, width=LINE_W)
        tag_y_label = format_krabiad(TAG_OFFSET_IN)
        draw_label(draw, tag_y_label, tag_line_x + TICK + 35,
                   (TAG_COLLAR_Y_FRONT + ty) // 2, font, fill=label_fill)
        draw_label(draw, "centered", tx + tw // 2, ty + th +
                   FONT_SIZE // 2 + 4, font, fill=label_fill)
    ...
    if tag_panel_w_raw:
        ...
        if TAG_IMAGE_PATH and Path(TAG_IMAGE_PATH).exists():
            ...
            tag_panel_img = Image.new(
                "RGBA", (tag_panel_w, tag_panel_h), (210, 210, 210, 200))
    ...
    draw_dimension_bracket(draw2, elem_x, sg_y + side_gh + PAD,
                           ...
                           w_label, font, 'horizontal', label_fill=label_fill)
    draw_dimension_bracket(draw2, elem_x + side_gw + PAD, sg_y,
                           elem_x + side_gw + PAD, sg_y + side_gh,
                           h_label, font, 'vertical', label_fill=label_fill)
...
def annotate(input_path: str, output_path: str | None = None, *,
    ...
    Returns a dict with:
      annotated     — Path to the annotated preview PNG
      mockup        — Path to the mockup PNG, or None if skipped
      side          — 'front' or 'back'
      measurements  — summary metrics (size, tier, offsets, labels)
    """
    ...
    _text_color = text_color if text_color is not None else TEXT_COLOR
    label_fill = _TEXT_COLORS.get(_text_color, _TEXT_COLORS["black"])

    is_url = input_path.startswith(
        "http://") or input_path.startswith("https://")
    ...
    if is_url:
        ...
        out = Path(output_path) if output_path else src / \
            (url_name + "_annotated.png")
        mockup_src_stem = url_name
    else:
        src = Path(input_path)
        if not src.exists():
            raise FileNotFoundError(f"Not found: {input_path}")
        out = Path(output_path) if output_path else src.with_stem(
            src.stem + "_annotated")
    ...
    print(
        f"  Image size : {W} × {H} px  ({PHYSICAL_W_IN} × {PHYSICAL_H_IN:.2f} in)")
    ...
    print(
        f"  X offset   : {x_offset_in:.2f} in  ({format_krabiad(x_offset_in)}) {'' if x_dir == 'centered' else x_dir}")
    print(f"{'─'*48}\n")
    ...
    # 0. Print area border
    draw.rectangle([ox, oy, ox + pw - 1, oy + ph - 1],
                   outline=(180, 180, 180), width=1)
    ...
    draw_label(draw, x_label, (cx_s + nx_s) // 2,
               hy // 2 + 4, font, fill=label_fill)

    # 4. Size + tier label below the bounding box
    size_text = f"{format_krabiad(elem_w_in)} × {format_krabiad(elem_h_in)}  [{tier}]"
    draw_label(draw, size_text, (bx0 + bx1) // 2, by1 +
               PAD + FONT_SIZE // 2, font, fill=label_fill)
    ...
    if not is_url:
        try:
            import rgb_to_cmyk as cmyk
            cmyk_out = cmyk.convert(str(src))
        except Exception as exc:
            print(f"  [cmyk] Skipped: {exc}")
    ...
    return {
        "annotated": out,
        "mockup": mockup_out if mockup_ok else None,
        "cmyk": cmyk_out,
        "side": _side,
...
def combine_mockups(paths, output_path=None, gap=COMBINE_GAP):
    ...
    split_x = _MOCKUP_W
    shirts = [img.crop((0, 0, min(split_x, img.width), img.height))
              for img in images]
    panels = [img.crop((min(split_x, img.width), 0, img.width, img.height))
              for img in images]
...
def run_batch_entries(sides, output_path=None) -> dict:
    """Run batch pipeline; return paths to annotated, mockup, and optional combined outputs.

    Returns:
      {
        "sides": [annotate() result dicts, ...],
        "combined": Path | None,
      }
    """
    if not sides:
        raise ValueError("run_batch_entries: sides list is empty.")
    ...
    for entry in sides:
        ...
        if result.get("mockup"):
            mockup_paths.append(result["mockup"])

    combined = None
    if len(mockup_paths) >= 2:
        if output_path is None:
            first = Path(sides[0]["input"])
            output_path = first.with_stem(first.stem + "_combined")
        combined = combine_mockups([str(p) for p in mockup_paths], output_path)
    elif len(mockup_paths) == 1 and output_path is not None:
        # Explicit combined path requested with a single side — skip combine
        pass

    return {"sides": side_results, "combined": combined}


def run_batch(sides, output_path=None):
    """Full pipeline: annotate each side, then combine into one image."""
    run_batch_entries(sides, output_path)
